...
BMSModule::BMSModule() {
  ...
  goodPackets = 0;
  badPackets = 0;
  balanceStartVoltage = 3.9f;  // 3.825
  balanceHyst = 0.003f;
  balanceStop = balanceStartVoltage - balanceHyst;
}
  ...
  if ((retLen == 22) && (buff[21] == calcCRC)) {
    if (buff[0] == (moduleAddress << 1) && buff[1] == REG_GPAI && buff[2] == 0x12)  //Also ensure this is actually the reply to our intended query
    {
      ...
      for (int i = 0; i < 6; i++) {
        ...
        if (cellVolt[i] > hiCell) hiCell = cellVolt[i];
        if (cellVolt[i] < loCell) loCell = cellVolt[i];
      }
      moduleCellDelta = hiCell - loCell;
...
float BMSModule::getLowCellV() {
  float lowVal = 10.0f;
  for (int i = 0; i < 6; i++)
    if (cellVolt[i] < lowVal) lowVal = cellVolt[i];
  return lowVal;
}
...
float BMSModule::getCellDeltaV() {
  //return getHighCellV() - getLowCellV();
  return moduleCellDelta;
}
...
void BMSModule::balanceCells() {
  ...
  for (int i = 0; i < 6; i++) {
    bool tempBalState = false; //balanceState[i];
    float cellVoltage = getCellVoltage(i);

    if (cellVoltage > balanceStartVoltage) {
      tempBalState = true;
    }

    if (cellVoltage < balanceStop) {  // Need current state?
      tempBalState = false;
    }
    ...
    balanceState[i] = tempBalState ? 1 : 0;
  }
...
void BMSModule::setBalanceStartVoltage(float value) {
  if (value < 3.2f || value > 4.2f) {
    Logger::info("Balance value out of range; must be 3.2V - 4.2V");
    return;
  }
  balanceStartVoltage = value;
  balanceStop = balanceStartVoltage - balanceHyst;
  setBalanceHyst(balanceHyst);  // Verify value with hyst
}

float BMSModule::getBalanceStartVoltage() {
  return balanceStartVoltage;
}

void BMSModule::setBalanceHyst(float value) {
  balanceHyst = value;
  balanceStop = balanceStartVoltage - balanceHyst;
  if (balanceStop < 3.2f) {
    Logger::info("Balance start value (with hyst) out of range; must be 3.2V - 4.2V. Setting stop to 3.2V");
    balanceStop = 3.2f;
  }
}

float BMSModule::getBalanceHyst() {
  return balanceHyst;
}
